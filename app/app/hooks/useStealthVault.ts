...
import {
  ERC20_ABI,
  STEALTH_VAULT_ABI,
  STEALTH_VAULT_ADDRESS_LOCAL,
  STEALTH_VAULT_ADDRESS_PASSET,
  TEST_ASSET_ID,
  TEST_TOKEN_ADDRESS_LOCAL,
  TEST_TOKEN_ADDRESS_PASSET,
} from "../lib/contracts/stealthVault";
import { config, hardhatLocal, polkadotHubTestnet } from "../lib/wagmi";
...
const NETWORK_CONFIGS: Record<number, NetworkConfig> = {
  [hardhatLocal.id]: {
    ...
    vaultAddress: STEALTH_VAULT_ADDRESS_LOCAL,
    tokenAddress: TEST_TOKEN_ADDRESS_LOCAL,
  },
...
const SUPPORTED_CHAIN_IDS = Object.keys(NETWORK_CONFIGS).map((id) => Number(id));
...
export function useStealthVault() {
  ...
  const ensureReady = useCallback(async () => {
    if (!publicClient) {
      throw new Error("Public client unavailable. Ensure your wagmi config includes this chain.");
    }
    ...
    const preferredChainId = chainId ?? walletClient?.chain?.id ?? hardhatLocal.id;
    const networkConfig = NETWORK_CONFIGS[preferredChainId];

    if (!networkConfig) {
      throw new Error("Unsupported network. Please switch to Hardhat Localhost or Polkadot Hub TestNet.");
    }

    if (!networkConfig.vaultAddress) {
      throw new Error(`StealthVault address not configured for ${networkConfig.label}.`);
    }

    if (!networkConfig.tokenAddress) {
      throw new Error(`Test token address not configured for ${networkConfig.label}.`);
    }
    ...
    if (!activeWalletClient) {
      try {
        ...
        throw new Error("Wallet client unavailable. Please try reconnecting your wallet.");
      }
    }
    ...
    if (walletChainId && walletChainId !== networkConfig.chainId) {
      throw new Error(`Please switch your wallet to ${networkConfig.label} (chainId ${networkConfig.chainId}).`);
    }
  ...
  const depositStealth = useCallback(
    async (args: DepositArgs) => {
      ...
      if (!vaultAddress || !tokenAddress) {
        throw new Error("StealthVault or token address missing for current network.");
      }
  ...
  const withdraw = useCallback(
    async (args: WithdrawArgs) => {
      ...
      const hash = await client.writeContract({
        ...
        args: [args.stealthId, TEST_ASSET_ID, recipient, args.amount],
        account,
      });
